...
\title{\huge\bfseries Évaluation\\ Séance 3}
\begin{document}
\maketitle

\begin{enumerate}
	\item Soient $a,b,c$ trois réels avec $a\neq 0$. On considère le polynôme $P(x)=ax^2+bx+c$. En écrire la forme canonique en faisant apparaître les étapes de calcul et l'idée importante.
	\item Écrire les polynômes suivants sous forme canonique. Factoriser quand c'est possible.
	\begin{enumerate}
		\item $5x^2-3x-2$
		\item $x^2-x+6$
		\item $2016x^2+2015$
	\end{enumerate}
	\item Qu'est-ce qu'une suite stationnaire ?
	\item Qu'est-ce qu'une suite formellement ?
	\item Factoriser $x^8-1$ le plus possible.
	\item Soit $a,b$ deux réels. Développer $(a+1)(b+1)^3$.
	\item Donner les règles sur les puissances qui figurent dans le Rappel Séance 0.
	\item La suite $(u_n)$ est définie pour tout entier naturel $n\geq 0$ par $u_n=n^2+n+1$. Donner l'expression de $u_{n+1}$. Montrer que pour tout $n\geq 0$, on a $u_n>0$.
\end{enumerate}
